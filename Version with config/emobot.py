...
hands = mp_hands.Hands(
    static_image_mode=config["static_image_mode"],          # Использование видеопотока или статичных изображений
    max_num_hands=config["max_num_hands"],                  # Максимальное количество рук для отслеживания
    min_detection_confidence=config["min_detection_confidence"],     # Минимальная уверенность для обнаружения руки
    min_tracking_confidence=config["min_tracking_confidence"]       # Минимальная уверенность для отслеживания руки
)
mp_drawing = mp.solutions.drawing_utils  # Утилиты для рисования распознанных ключевых точек

def recognize_emotion(frame):
    try:
        # DeepFace ожидает изображение в формате RGB
        ...
        # Проверяем, является ли результат списком (несколько лиц) или словарем (одно лицо)
        if isinstance(analysis, list):
            # Если обнаружено несколько лиц, берем первое
...
class Emoji:
    ...
    """
    def __init__(self, x, y, emoji, radius=30):
        ...
        self.radius = radius  # Радиус эмодзи для проверки коллизий
    ...
    def update(self, gravity, width, height):
        """
...
class EmojiGameApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(config["window_title"])  # Устанавливаем заголовок окна из конфигурации
        self.setGeometry(*config["window_geometry"])  # Устанавливаем позицию и размер окна из конфигурации
        ...
        self.cap = cv2.VideoCapture(config["camera_index"])  # Открываем веб-камеру по индексу из конфигурации
        ...
        self.timer.start(int(1000 / config["video_frame_rate"]))  # Запускаем таймер с интервалом, основанным на частоте кадров из конфигурации

        # Настройка эмодзи
        self.emojis = config["emojis"]  # Список возможных эмодзи из конфигурации
        self.emoji_list = []  # Список текущих эмодзи на экране

        # Инициализация переменной для хранения текущей эмоции
        self.current_emotion = "Н/Д"

        # Счётчик кадров для оптимизации распознавания эмоций
        self.frame_count = 0
    ...
    def update_frame(self):
        ...
        # Распознавание эмоций раз в 10 кадров для оптимизации
        self.frame_count += 1
        if self.frame_count % 10 == 0:
            self.current_emotion = recognize_emotion(image)

        # Случайное появление нового эмодзи
        if random.random() < config["emoji_spawn_probability"]:  # С вероятностью из конфигурации
        ...
        for emoji in self.emoji_list:
            emoji.update(gravity=config["emoji_gravity"], width=width, height=height)  # Применяем гравитацию из конфигурации и обновляем позицию
                    ...
                    if collision_point:
                        ...
                        if is_left:
                            ...
                            if distance > 5:  # Если эмодзи не совсем на пальце, притягиваем его
                                force = config["emoji_gravity"]  # Сила притяжения из конфигурации
        ...
        # Отображение текущей эмоции на экране
        painter.setPen(QPen(Qt.yellow, 2, Qt.SolidLine))
        painter.setFont(QFont("Arial", 24))
        painter.drawText(10, 30, f"Эмоция: {self.current_emotion}")  # Рисуем текст с текущей эмоцией
